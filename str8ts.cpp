...
#include <vector>
#include <string>
#include "json.hpp" // Include the JSON library (make sure to link with -ljsoncpp)
#include <sstream>
#include <iterator>
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <easy_games> <medium_games> <hard_games>\n";
        return 1;
    }

    // Parse command-line arguments
    int easyCount = std::stoi(argv[1]);
    int mediumCount = std::stoi(argv[2]);
    int hardCount = std::stoi(argv[3]);

    // JSON object to store all games
    json allGames;

    // Helper lambda to generate games
    auto generateGames = [&](int count, int difficulty, const std::string& difficultyLabel) {
        for (int i = 0; i < count; ++i) {
            std::vector<Field> game;
            generate(game, difficulty);
            std::string encodedGame = encodeGame(game);
            allGames[difficultyLabel].push_back(encodedGame);
        }
    };

    // Generate games for each difficulty
    generateGames(easyCount, 1, "easy");
    generateGames(mediumCount, 2, "medium");
    generateGames(hardCount, 3, "hard");

    // Write to JSON file
    std::ofstream outFile("games.json");
    outFile << allGames.dump(4); // Indented with 4 spaces
    outFile.close();

    std::cout << "Games have been generated and saved to config.json\n";
    return 0;
}